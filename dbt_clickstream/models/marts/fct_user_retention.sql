...
cohort_base as (
    select date_trunc('day', signup_date) as cohort_date,
      ...
      from user_session
     group by date_trunc, variant, acquisition_channel
)
...
select c.cohort_date, c.variant, c.acquisition_channel, c.cohort_size,
       ...
       round(100.0 * count(distinct case when s.days_since_signup = 1 then s.user_id end)/ nullif(c.cohort_sized, 0), 2) as d1_retention_rate,
 ...
 from cohort_base as c left join user_sessions as s on  c.cohort_date = date_trunc('day', s.signup_date)
